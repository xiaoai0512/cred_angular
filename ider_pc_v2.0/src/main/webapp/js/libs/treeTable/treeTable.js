...
layui.define(['layer', 'laytpl', 'form'], function (exports) {
    ...
    $.ajax({
    	...
    	url: ctx+'/js/libs/treeTable/treeTable.css',
        async: false,
    ...
    var TreeTable = function (options) {
        ...
        var defaultOption = {
            ...
            getAllChooseBoxDis: function () {  // 获取全选按钮不可编辑
                return getAllChooseBoxDis(this);
            },
        ...
        var textDefaultOption = {
            none: '<div style="padding: 2px 0; background: #F3F3F3;">暂无数据</div>'  // 空文本提示文字
        };
    ...
    TreeTable.prototype.renderBodyTr = function (d, indentSize, isHide, $tr) {
        ...
        if(haveChild == false && checkFlag == false){
        	cols[0].type = undefined;
        }
        if ($tr) {
...
//        	}
    ...
    TreeTable.prototype.renderBodyData = function (data, d, $tr) {
        ...
        if ($tr) {
            // 更新父级复选框状态  ky注释，目前系统不需要级联，有需要可以放开，会影响资产选择===
       ...
       // this.checkChooseAllCB();  // 联动全选框  ky注释，目前系统不需要级联，有需要可以放开，会影响资产选择===
        updateFixedTbHead($view);
    };
    ...
    /** 设置复/单选框选不可编辑 */
    TreeTable.prototype.setCheckedDis = function () {
        var components = this.getComponents();
        var $table = components.$table;
        var checkboxFilter = components.checkboxFilter;
        var radioFilter = components.radioFilter;
...
//            }
    ...
    function getThead(options) {
        ...
        for (var i = 0; i < options.cols.length; i++) {
            ...
            if (col.type == 'checkbox') {
            	if(col.clssType == 'disabled'){
            		htmlStr += options.getAllChooseBoxDis();
            	}else{
    ...
    /** 生成全选按钮 ====核算用不可编辑*/
    function getAllChooseBoxDis(options) {
        var tbFilter = $(options.elem).next().attr('lay-filter');
        var cbAllFilter = 'ew_tb_choose_all_' + tbFilter;
        return '<input type="checkbox" lay-filter="' + cbAllFilter + '" lay-skin="primary" class="ew-tree-table-checkbox" disabled/>';
    }
