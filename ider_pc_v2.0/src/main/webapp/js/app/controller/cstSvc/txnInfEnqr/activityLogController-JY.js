...
define(function(require) {
	...
	//交易类活动日志查询
	webApp.controller('txnCgyAvyLogEnqrCtrl-JY', function($scope, $stateParams,
		...
		/*$scope.revocation = function(item){
			$scope.params = {
					ecommEntryId:  $scope.txnCgyAvyLogEnqrTable.params.externalIdentificationNo,
					ecommTransCurr:  item.transCurrCde,
					ecommTransAmount: item.transAmount,
					ecommPostingCurr:  item.accountCurrCde,
					ecommTransPostingAmount:  item.accountAmount,
					ecommTransDate:  item.transDate,
					ecommGlobalSerialNumbr:  item.globalSerialNumbr
			};
			jfRest.request('txnCgyAvyLogEnqr', 'revocationTxn', $scope.params).then(function(data) {
				if (data.returnCode == '000000') {
					jfLayer.success("撤销成功！");
