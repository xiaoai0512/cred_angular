...
define(function(require) {
	...
	// 利息信息查询
	webApp.controller('interestCreditCtrl', function($scope, $stateParams, jfRest,
		...
		$scope.costInfList = {
				...
				paging : true,
				resource : 'costInfMag.query',//accBscInf.queryInterest
				autoQuery : false,
		...
		$scope.adjustAmtBtn = function(item){
			...
			$scope.modal('/cstSvc/interestQuery/interestAdjustAmount.html', $scope.item, {
		...
		$scope.sureAdjustAmt = function(result){
            ...
            if($scope.item.adjustAmount == null || $scope.item.adjustAmount == undefined){
...
//				ecommEntryId:	$scope.item.externalIdentificationNo,
			...
			//消费杂费实时贷调事件
			jfRest.request('interest','changeEv',$scope.adjustParams,'',$scope.eventNoTrends).then(function(data) {
				if (data.returnCode == '000000') {
		...
		//取现手续费实时贷调事件 
		$scope.cashInterestHandle = function(params,result){
			jfRest.request('interest', 'cashInterest', params).then(function(data) {
				if (data.returnCode == '000000') {
					$scope.costInfList.params = {
						idType: $scope.costSearch.idType,
						idNumber: $scope.costSearch.idNumber,
						externalIdentificationNo: $scope.costSearch.externalIdentificationNo,
						pageSize : 10,
	...
	//贷记调整金额
	webApp.controller('interestAdjustAmountCtrl', function($scope, $stateParams, jfRest,
			$http, jfGlobal, $rootScope, jfLayer, $location,lodinDataService,$translate,$translatePartialLoader,T) {
		$scope.lang = window.localStorage['lang'];
		$translate.use($scope.lang);
		$translatePartialLoader.addPart('pages/cstSvc/costInfMage/i18n_costInf');
