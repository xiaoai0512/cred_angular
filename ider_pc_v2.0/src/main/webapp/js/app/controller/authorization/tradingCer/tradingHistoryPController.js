...
define(function(require) {
	...
	webApp.controller('tradingHistoryCtrl', function($scope, $stateParams, jfRest,
		...
		form.on('select(getIdType)',function(data){
			if(data.value == "1"){//身份证
				$("#tradingHistoryP_idNumber").attr("validator","id_idcard");
			}else if(data.value == "2"){//港澳居民来往内地通行证
				$("#tradingHistoryP_idNumber").attr("validator","id_isHKCard");
			}else if(data.value == "3"){//台湾居民来往内地通行证
				$("#tradingHistoryP_idNumber").attr("validator","id_isTWCard");
			}else if(data.value == "4"){//中国护照
				$("#tradingHistoryP_idNumber").attr("validator","id_passport");
			}else if(data.value == "5"){//外国护照passport
				$("#tradingHistoryP_idNumber").attr("validator","id_passport");
			}else if(data.value == "6"){//外国人永久居留证
				$("#tradingHistoryP_idNumber").attr("validator","id_isPermanentReside")
            }
        });
		...
		$scope.itemList = {
				...
				autoQuery:false,
				resource : 'tradingHistoryList.query',// 列表的资源
		...
		$scope.selectInfo = function(event) {
			...
			// 页面弹出框事件(弹出页面)
			$scope.item = event;
			$scope.sId = $scope.item.id;
			$scope.modal('/authorization/tradingList/tradingHistoryInfo.html', $scope.item, {
				buttons : [ T.T('F00012')],
				size : [ '950px', '530px' ],
	...
	webApp.controller('HistoryInfoCtrl', function($scope, $stateParams, jfRest,
		...
		//授权检查结果
				...
				if($scope.verifnArea.ecommVerifnRsltCvv2 == 'A'){
					$scope.item.ecommVerifnRsltCvv2Info = T.T('SQJ2700001');
