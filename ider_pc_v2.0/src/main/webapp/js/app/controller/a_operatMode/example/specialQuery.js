...
define(function(require) {
	...
	//构件实例查询及维护
	webApp.controller('specialQueryCtrl', function($scope, $stateParams, jfRest,
...
//		运营模式
		...
		$scope.updateArtifactInstan = function(item) {
			// 页面弹出框事件(弹出页面)
			$scope.items = $.parseJSON(JSON.stringify(item));
			$scope.modal('/a_operatMode/example/updateSpecial.html', $scope.items, {
				title : T.T('YYJ400041'),
		...
		$scope.artifactAdd = function(){
			// 页面弹出框事件(弹出页面)
			$scope.modal('/a_operatMode/example/specialExampleEst.html', '', {
				title : T.T('YYJ600018'),
				buttons : [T.T('F00107'),T.T('F00012')],
				size : [ '1150px', '500px' ],
		...
		$scope.saveArtifactAdd = function(result){
			...
			if($scope.artifactExampleInf.instanDimen1 && $scope.artifactExampleInf.instanDimen1 !="null"){
				if($scope.artifactExampleInf.instanCode1 ==null){
					jfLayer.fail(T.T('YYJ400058'));//实例代码为空
	        		return;
				}		
			}
			if($scope.artifactExampleInf.instanDimen2 && $scope.artifactExampleInf.instanDimen2 !="null"){
				if($scope.artifactExampleInf.instanCode2 ==null){
					jfLayer.fail(T.T('YYJ400059'));//实例代码为空
	        		return;
				}
			}
			if($scope.artifactExampleInf.instanDimen3 && $scope.artifactExampleInf.instanDimen3 !="null"){
				if($scope.artifactExampleInf.instanCode3 ==null){
					jfLayer.fail(T.T('YYJ400060'));//实例代码为空
	        		return;
				}
			}
			if($scope.artifactExampleInf.instanDimen4 && $scope.artifactExampleInf.instanDimen4 !="null"){
				if($scope.artifactExampleInf.instanCode4 ==null){
					jfLayer.fail(T.T('YYJ400061'));//实例代码为空
	        		return;
				}
			}
			if($scope.artifactExampleInf.instanDimen5 && $scope.artifactExampleInf.instanDimen5 !="null"){
				if($scope.artifactExampleInf.instanCode5 ==null){
					jfLayer.fail(T.T('YYJ400062'));//实例代码为空
	        		return;
				}
			}
			if(result.scope.pcdExampleInf.baseInstanDimen && result.scope.pcdExampleInf.baseInstanDimen !="null"){
				if(result.scope.pcdExampleInf.baseInstanCode ==null){
					jfLayer.fail(T.T('YYJ400053'));//实例代码为空
	        		return;
				}
			}
			if(result.scope.pcdExampleInf.optionInstanDimen && $scresult.scopeope.pcdExampleInf.optionInstanDimen !="null"){
				if(result.scope.pcdExampleInf.optionInstanCode ==null){
	...
	webApp.controller('specialUpdateCtrl', function($scope,$timeout, $stateParams, jfRest,
			$http, jfGlobal, $rootScope, jfLayer, $location,lodinDataService,$translate,$translatePartialLoader,T) {
		$scope.items.artifactInstanId = $scope.items.id;
		//pcd实例默认不显示
		...
		$scope.segmentTypeArray ={ 
	        ...
	        callback: function(data){
	        	$scope.segmentTypeUS = $scope.pcdExampleInf.segmentType;
	        }
		};
		//pcd实例化取值类型
		$scope.pcdtypeArray = {
			...
			resource: "paramsManage.query", //数据源调用的action 
			callback: function(data) {
				$scope.pcdTypeUS = $scope.pcdExampleInf.pcdType;
				$timeout(function() {
		...
		//获取修改后的取值类型
        var form = layui.form;
		 form.on('select(getpcdTypeU)',function(event){
			 $scope.pcdTypeUS = event.value;
		 });
       ...
       $scope.saveNewAdrInfo = function() {
    	   if(null== $scope.pcdExampleInf.pcdPoint  || null== $scope.pcdTypeUS 
			...
			pcdInfTableInfoU.pcdType = $scope.pcdTypeUS;
			...
			if($scope.indexNo!= undefined && $scope.indexNo!=null){
				...
				$scope.pcdInfTable[$scope.indexNo].pcdType = 	 $scope.pcdTypeUS;
	...
	// 构件实例查询
	webApp.controller('specialExampleExtCtrl', function($scope, $stateParams, jfRest,$http, jfGlobal, $rootScope, jfLayer, $location,lodinDataService,$translate,$translatePartialLoader,T) { 
		$scope.lang = sessionStorage.getItem(sessionStorage.getItem("userName")+"_lang");
		$translate.use($scope.lang);
		$translatePartialLoader.addPart('pages/a_operatMode/example/i18n_artifact');
		$translate.refresh();
		...
		//构件编号
		 $scope.artifactNoArr ={ 
			...
			$scope.chosedArtifact = function (result){
				...
				$scope.artifactNoInfo = $scope.checkedArtifactInf.artifactNo + $scope.checkedArtifactInf.artifactDesc;
...
//				$scope.InstanCodeValue(dataValueCount,$scope.checkedEvent.eventNo);
		...
		$scope.queryPcd = function(dataValue){
			...
			jfRest.request('pcd', 'query', $scope.queryPcdParam).then(function(data) {
				if (data.returnCode == '000000') {
					if(data.returnData.rows!=null){
						//pcd实例显示pcdExampleInf.baseInstanDimen
						$scope.pcdInstanShow = true;
						$scope.pcdExampleInf.pcdNo = dataValue.substring(0,8);
						$scope.pcdExampleInf.baseInstanDimen = data.returnData.rows[0].baseInstanDimen;
